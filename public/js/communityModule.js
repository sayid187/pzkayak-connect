 ...
 * Tablas Supabase: amigos, actividades, actividad_participantes, mensajes
 */
...
const communityModule = {
    ...
    async cargarTodo() {
        await Promise.all([this.cargarPerfil(), this.cargarAmigos(), this.cargarActividades()]);
    ...
    async cargarCercanos() {
        try {
            const idsExcluir = [...this.amigos.map(a => a.amigo_id), this.currentUser.id];
            ...
            this.cercanos = (data || [])
                .map(p => ({
                    id: p.id, nombre: p.nombre || 'Pescador',
    ...
    async crearActividadDb(datos) {
        const { data, error } = await db.from('actividades').insert({
            creador_id: this.currentUser.id, titulo: datos.titulo,
            lugar: datos.lugar, fecha: datos.fecha, hora: datos.hora,
            max_participantes: datos.max, descripcion: datos.descripcion || '',
        }).select().single();
        if (error) throw error;
    ...
    detenerCompartirUbicacion() {
        ...
        this.ubicWatchId = null;
        this.ubicInterval = null;
    ...
    async guardarUbicacion(lat, lng, compartir) {
        ...
        try {
            await db.from('perfiles').upsert({
                id: this.currentUser.id, lat, lng,
    ...
    abrirChat(amigo) {
        ...
        overlay.className = 'fixed inset-0 z-50 flex flex-col';
        const isDark = document.body.classList.contains('dark');
        overlay.style.background = isDark ? '#0f172a' : '#f1f5f9';

        overlay.innerHTML = `
            <div id="chat-header" style="background:${isDark?'#1e293b':'white'};border-bottom:1px solid ${isDark?'#334155':'#e5e7eb'};padding:12px 16px;display:flex;align-items:center;gap:12px;flex-shrink:0;">
                <button id="chat-back" style="color:#6b7280;padding:4px;background:none;border:none;cursor:pointer;">
                ...
                <div class="w-9 h-9 ${this.colorPara(amigo.nombre)} rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 relative">
                    ...
                    <span class="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${amigo.online ? 'bg-green-500' : 'bg-gray-300'}"></span>
                </div>
                <div class="flex-1 min-w-0">
                    <p style="font-weight:600;color:${isDark?'#e2e8f0':'#1f2937'}">${amigo.nombre}</p>
                    <p style="font-size:12px;color:#9ca3af;">${amigo.online ? '🟢 En línea' : '⚫ Desconectado'}</p>
                </div>
            </div>
            ...
            <div id="chat-input-wrap" style="background:${isDark?'#1e293b':'white'};border-top:1px solid ${isDark?'#334155':'#e5e7eb'};padding:10px 14px;display:flex;gap:8px;align-items:flex-end;flex-shrink:0;padding-bottom:max(10px,env(safe-area-inset-bottom));">
    ...
    async cargarMensajes(amigoId) {
        try {
            ...
            if (error) throw error;
            this.mensajes = data || [];
            await db.from('mensajes').update({ leido: true })
                .eq('receptor_id', uid).eq('emisor_id', amigoId).eq('leido', false);
        } catch { this.mensajes = []; }
        this.renderMensajes();
    },
    ...
    async enviarMensaje() {
        ...
        try {
            const { data, error } = await db.from('mensajes').insert({
                emisor_id: this.currentUser.id, receptor_id: this.chatAbierto.amigo_id, texto, leido: false
            }).select().single();
            if (error) throw error;
            ...
            this.mensajes = this.mensajes.filter(m => !m._pendiente);
            this.renderMensajes();
        }
    },
    ...
    renderMensajes() {
        ...
        if (this.mensajes.length === 0) {
            cont.innerHTML = `<div style="text-align:center;color:#9ca3af;margin:auto;padding:32px;">
                ...
                <p>Sin mensajes aún</p><p style="font-size:12px;margin-top:4px;">¡Di hola! 🎣</p>
            </div>`;
            return;
        }
        ...
        this.mensajes.forEach(msg => {
            ...
            burbuja.innerHTML = `
                ...
                <span style="font-size:10px;color:#9ca3af;margin-top:2px;padding:0 4px;">
                    ${hora}${esPropio?(msg._pendiente?' ⏳':msg.leido?' ✓✓':' ✓'):''}
                </span>`;
    ...
    estaOnline(ultimaVez) {
        ...
        return Date.now() - new Date(ultimaVez).getTime() < 15 * 60 * 1000;
    },
    ...
    cargarDatosDemo() {
        ...
        this.actividades = [{
            id:'demo1', titulo:'Pesca de Fin de Semana', fecha:'2025-07-26', hora:'08:00',
            lugar:'Zona costera', participantes:[], maxParticipantes:10, creador:'Carlos', descripcion:''
        }];
    },
    ...
    mostrarMapa(lat, lng, zoom = 13) {
        const c = document.getElementById('community-map');
        if (!c) return;
    ...
    renderAmigos() {
        ...
        if (this.amigos.length === 0) {
            cont.innerHTML = `<div class="text-center py-6 text-gray-400">
                <i class="fa fa-users text-3xl mb-2 block"></i>
                <p class="text-sm">Sin amigos aún</p>
                <p class="text-xs mt-1">Añade pescadores desde la lista de cercanos</p></div>`;
            return;
        }
    ...
    renderCercanos() {
        ...
        if (this.cercanos.length === 0) {
            cont.innerHTML = `<div class="text-center py-6 text-gray-400">
                ...
                <p class="text-sm">Sin pescadores cercanos con ubicación activa</p></div>`;
            return;
        }
    ...
    async agregarAmigo(pescador) {
        if (this.amigos.some(a => a.amigo_id === pescador.id)) { toast(`${pescador.nombre} ya es tu amigo`, 'info'); return; }
        try {
            await this.agregarAmigoDb(pescador.id);
            await this.cargarAmigos();
            this.cercanos = this.cercanos.filter(p => p.id !== pescador.id);
            this.render();
            toast(`¡${pescador.nombre} añadido! 🎣`, 'success');
        } catch (err) { toast('Error: ' + err.message, 'error'); }
    },
    ...
    renderActividades() {
        ...
        if (this.actividades.length === 0) {
            cont.innerHTML = `<div class="text-center py-6 text-gray-400">
                ...
                <p class="text-sm">Sin actividades — ¡crea una!</p></div>`;
            return;
        }
            ...
            item.innerHTML = `
                ...
                <div class="mb-3">
                    <div class="flex justify-between text-xs text-gray-400 mb-1">
                        <span>${act.participantes.length} participantes</span><span>Máx. ${act.maxParticipantes}</span>
                    </div>
                ...
                <div class="flex gap-2">
                    ${!esFuturo&&!esHoy?'':!yaUnido&&!lleno
                        ...
                        :yaUnido
                            ?`<button class="btn btn-secondary text-sm flex-1 btn-salir"><i class="fa fa-check mr-1"></i> Unido — Salir</button>`
                            :`<button class="btn text-sm flex-1 bg-gray-200 text-gray-400 cursor-not-allowed" disabled><i class="fa fa-lock mr-1"></i> Lleno</button>`
                    }
